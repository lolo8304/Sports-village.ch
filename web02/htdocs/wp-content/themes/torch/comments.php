<?php
	...
	if ( post_password_required() ) { ?>
		<p class="nocomments"><?php _e('This post is password protected. Enter the password to view comments.', 'torch'); ?></p> 
	<?php
		return;
	}
?>
...
<?php if ( have_comments() ) : ?>
	<h3 id="comments"><?php comments_number(__('No comment', 'torch'), __('Has one comment', 'torch'), __('% comments', 'torch'));?> <?php printf(__('to &#8220;%s&#8221;', 'torch'), the_title('', '', false)); ?></h3>
<div class="upcomment"><?php _e('You can ','torch'); ?><a id="leaverepond" href="#comments"><?php _e('leave a reply','torch'); ?></a>  <?php _e(' or ','torch'); ?> <a href="<?php trackback_url(true); ?>" rel="trackback"><?php _e('Trackback','torch'); ?></a> <?php _e('this post.','torch'); ?></div>
	<ol id="thecomments" class="commentlist comments-list">
	<?php wp_list_comments('type=comment&callback=torch_comment');?>
	</ol>
...
?>
		<div id="commentnavi">
			<span class="pages"><?php _e('Comment pages', 'torch'); ?></span>
			<div id="commentpager">
 ...
 <?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ( comments_open() ) : ?>
	 ...
	 <?php else : // comments are closed ?>
		...
		<p class="nocomments"><?php //_e('Comments are closed.', 'torch'); ?></p>
...
<p><?php printf(__('You must be <a href="%s">logged in</a> to post a comment.', 'torch'), wp_login_url( get_permalink() )); ?></p>
<?php else : ?>
<?php 
...
'class_submit'         => 'submitss',
    ...
    __( 'Your email address will not be published.', 'torch' ) . ( $req ? $required_text : '' ) .
    '</p>',
        // change the title of the reply section
        'title_reply'=>__('Write a Reply or Comment', 'torch'),
